...
const getStatus = require('../src/plugins/transactions-list/transaction-status')
...
describe('Transaction status', function() {
  ...
  it('should report success status on pre-Byzantine fork', function() {
    ...
    getStatus(
      { input, gas },
      { blockNumber, gasUsed, logs, status }
    ).should.equal(true)
  ...
  it('should report failure status on pre-Byzantine fork', function() {
    ...
    getStatus(transaction, receipt).should.equal(false)
  })
