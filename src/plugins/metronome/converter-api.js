...
function getCoinToMetEstimate(coin) {
  const met = createMetronome(createProvider.fromLib(coin.lib))
  return ({ value }) =>
    met.getCoinsToMetResult(value).then(result => ({ result }))
}

function getMetToCoinEstimate(coin) {
  const met = createMetronome(createProvider.fromLib(coin.lib))
  return ({ value }) =>
    met.getMetToCoinsResult(value).then(result => ({ result }))
}
...
function convertMet(web3, chain, logTransaction, metaParsers) {
  ...
  return function(privateKey, transactionObject) {
    ...
    const approvalMeta = _value =>
      metaParsers.approval({
    ...
    return Promise.all([
      ...
      if (toBN(remaining).gtn(0) && toBN(remaining).lt(toBN(value))) {
        return Promise.all([
          ...
          logTransaction(
            METToken.methods
              .approve(converterAddress, value)
